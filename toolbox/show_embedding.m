...
filename = 'colorimglist.txt';
fs = textread(filename, '%s');
N = length(fs);

%% create an embedding image

S = 480; % size of full embedding image
G = zeros(S, S-1, 3, 'uint8');
s = 40; % size of every single image
...
S = 480; % size of final image
G = zeros(S, S, 3, 'uint8');
s = 40; % size of every image thumbnail
