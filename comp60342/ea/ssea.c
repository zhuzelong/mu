...
/* My answer */
int mask[158]; // a uniform mask for crossover
...
void initmask()
{
    // Initialize a mask for crossover two genome
    // Mixing ratio is 0.5

    int i;

    int inited = 0;     // Indicate if there is a mask

    if(inited == 1)
    {
        printf("There is already a mask for crossover.\n");
        return;
    }
